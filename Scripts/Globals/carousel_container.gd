...
@export var wrapedound_radius: float = 300.0
@export var wrapadound_height : float = 50.00
...
func _process(delta: float) -> void:
	...
	for i in position_offset_node.get_children():
		if wraparound_enabled:
			...
			var x = sin(angle) * wrapedound_radius
			var y = cos(angle) * wrapadound_height
			var target_pos = Vector2(x, y-wrapadound_height) - i.size/2.0
